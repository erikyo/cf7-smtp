...
export function smtpAdmin(): void {
	...
	const updateUI = (method: string): void => {
		...
		document
			...
			.forEach((card) => {
				...
				// Prevent interacting with disabled cards visually
				if (input.disabled) {
		...
		if (method === 'wp') {
			classicFields.forEach((row) => {
				row.style.display = 'none';
				row.querySelectorAll('input, select, textarea').forEach((el) => ((el as HTMLInputElement).disabled = true));
			});
			oauthFields.forEach((row) => {
				row.style.display = 'none';
				row.querySelectorAll('input, select, textarea').forEach((el) => ((el as HTMLInputElement).disabled = true));
			});
			if (wpWarning) {
			...
			classicFields.forEach((row) => {
				...
				row.querySelectorAll('input, select, textarea').forEach((el) => {
					// Don't re-enable if it was disabled by a constant
					if (!el.hasAttribute('data-constant-disabled')) {
	...
	// Mark inputs that are already disabled on page load so we don't accidentally enable them
	document.querySelectorAll('input, select, textarea').forEach((el) => {
		if ((el as HTMLInputElement).disabled) {
			el.setAttribute('data-constant-disabled', 'true');
		}
	});
	...
	// Reset preset dropdown if manual edits occur
	const resetPreset = () => {
		if (formSelectDefault) {
			formSelectDefault.value = 'custom';
		}
	};
	if (formSelectHost) {
		formSelectHost.addEventListener('input', resetPreset);
	}
	if (formSelectPort) {
		formSelectPort.addEventListener('input', resetPreset);
	}
	document.querySelectorAll('input[name="cf7-smtp-options[auth]"]').forEach((radio) => {
		radio.addEventListener('change', resetPreset);
	});

	// Track dirty state for OAuth fields to prevent connecting with stale data
	let isOauthDirty = false;
	const oauthInputs = document.querySelectorAll('.cf7-smtp-oauth2-field');
	...
	if (connectBtn) {
		connectBtn.addEventListener('click', (e) => {
			...
			if (isOauthDirty) {
				alert(__('You have unsaved changes in your OAuth2 settings. Please save the settings first before connecting.', 'cf7-smtp'));
				return;
			}
	...
	// Debounce utility with a flush/cancel capability
	const createDebouncer = (func: () => void, wait: number) => {
		...
		const flush = function () {
			if (timeout) {
				...
				// Even if no timeout is active, we might want to trigger it if forced via blur
				// but to prevent double calling if it literally just ran, we rely on the lastCheckedEmail
				func();
			}
		};
	...
	if (fromMailInput) {
		...
		const { debounced, flush } = createDebouncer(checkFn, 1000); // 1 second debounce

		fromMailInput.addEventListener('keyup', debounced);
		fromMailInput.addEventListener('blur', flush);
	}
}
